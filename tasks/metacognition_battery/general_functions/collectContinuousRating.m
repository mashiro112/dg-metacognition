...
% drawWrappedUTF8 Render wrapped lines centered with UTF-8 drawing without
% relying on TextBounds (which can error on some setups).

    lineHeight = 28;
    [centerX, ~] = RectCenter(rect);
    for i_line = 1:numel(wrappedLines)
        approxWidth = length(wrappedLines{i_line}) * 14; % rough width to center text
        x = centerX - (approxWidth / 2);
        y = startY + ((i_line - 1) * lineHeight);
        Screen('DrawText', windowPtr, wrappedLines{i_line}, x, y, [255 255 255]);
    end
end

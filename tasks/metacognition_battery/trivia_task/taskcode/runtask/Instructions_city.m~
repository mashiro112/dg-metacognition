function Instructions_countries(p, list_cities_complete)

% load('list_cities_complete.mat');

countries_1_pos = 1; %London
countries_2_pos = 2; %Paris

%loading images
im_left = Screen('MakeTexture', p.window, list_cities_complete{countries_1_pos, 3});
im_right = Screen('MakeTexture', p.window, list_cities_complete{countries_2_pos, 3});
%  Assigning the word for the cities
word_left = list_cities_complete(countries_1_pos, 1);
word_right = list_cities_complete(countries_2_pos, 1);
word_left = char(word_left);
word_right = char(word_right);

Screen('TextSize',p.window,p.textSize);
DrawUTF8(p.window,['在本任务中，你将看到两座城市的图片和名称，如下所示。'...
    '\n\n每一试次请判断哪座城市的人口更多。'],...

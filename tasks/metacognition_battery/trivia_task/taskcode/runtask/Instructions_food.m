...
DrawUTF8(p.window,['请准备进入实验的第二部分。'...
    '\n\n按任意键继续。'],...
...
Screen('TextSize',p.window,p.textSize);
DrawUTF8(p.window,['在本任务中，你将看到两种食物的图片和名称，如下所示。'...
    '\n\n每一试次请判断哪种食物的热量更高。'],...
    'center',p.screenYpixels * 0.15, p.textColor);
...
if resp==1
    DrawUTF8(p.window,['你答对了！Kitkat 的热量高于西瓜。'],...
    'center',p.screenYpixels * 0.55, p.textColor);
elseif resp==2
    DrawUTF8(p.window,['很遗憾，这次不正确。Kitkat 的热量高于西瓜。'],...
    'center',p.screenYpixels * 0.55, p.textColor);
end

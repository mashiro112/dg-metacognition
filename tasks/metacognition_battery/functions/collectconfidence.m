...
curWindow = window;
center = [p.midW p.midH];
keys = [KbName('leftarrow') KbName('rightarrow');];

%% Initialise VAS scale
VASwidth=p.VASwidth_inPixels;
VASheight=p.VASheight_inPixels;
VASoffset=p.VASoffset_inPixels;
arrowwidth=p.arrowWidth_inPixels;
...
while (secs - start_time) < p.confDuration_inSecs;
    WaitSecs(.01);
